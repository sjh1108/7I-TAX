...
public class TaxBracketDataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        log.info("[TaxBracketDataInitializer] 2025년 종합소득세 세율 구간 데이터를 초기화합니다.");

        List<TaxBracket> brackets = List.of(
                TaxBracket.builder()
                        .year(2025).bracketMin(0).bracketMax(14_000_000L)
                        .rate(0.06).progressiveDeduction(0L).build(),
                TaxBracket.builder()
                        .year(2025).bracketMin(14_000_001L).bracketMax(50_000_000L)
                        .rate(0.15).progressiveDeduction(1_260_000L).build(),
                TaxBracket.builder()
                        .year(2025).bracketMin(50_000_001L).bracketMax(88_000_000L)
                        .rate(0.24).progressiveDeduction(5_760_000L).build(),
                TaxBracket.builder()
                        .year(2025).bracketMin(88_000_001L).bracketMax(150_000_000L)
                        .rate(0.35).progressiveDeduction(15_440_000L).build(),
                TaxBracket.builder()
                        .year(2025).bracketMin(150_000_001L).bracketMax(300_000_000L)
                        .rate(0.38).progressiveDeduction(19_940_000L).build(),
                TaxBracket.builder()
                        .year(2025).bracketMin(300_000_001L).bracketMax(500_000_000L)
                        .rate(0.40).progressiveDeduction(25_940_000L).build(),
                TaxBracket.builder()
                        .year(2025).bracketMin(500_000_001L).bracketMax(1_000_000_000L)
                        .rate(0.42).progressiveDeduction(35_940_000L).build(),
                TaxBracket.builder()
                        .year(2025).bracketMin(1_000_000_001L).bracketMax(9_999_999_999L)
                        .rate(0.45).progressiveDeduction(65_940_000L).build()
        );

        taxBracketRepository.saveAll(brackets);
        log.info("[TaxBracketDataInitializer] 세율 구간 {}개 저장 완료.", brackets.size());
    }
}
